package com.eurybus.exercise07;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // mp3 files recursively from sdcard ( takes time to make a list of songs in sdcard)
    ...
    //We are using asynctask to read all mp3 file names
    private class LoadSongsTask extends AsyncTask<Void, String, Void>{
        ...
        protected Void doInBackground(Void... url){
            updateSongsRecursive(new File(mediaPath));
            return null;
        }

        private void updateSongsRecursive(File path) {
            if(path.isDirectory()){
                for(int i = 0; i < path.listFiles().length; i++){
                    File file = path.listFiles()[i];
                    updateSongsRecursive(file);
                }
